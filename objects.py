...
class Card(abstract.AbstractImage) :
	...
	def getNumber(self) : return int(self.name)
...
class MainPile(abstract.AbstractTilePile) :
	...
	def topCardClicked(self, pos) :
		...
		for i, card in enumerate(self.cards) :
			if card.hasPosition(pos) : 
				result = i
				
		if result != len(self.cards)-1:
	...
	def validAddCards(self, cards) :
		...
		if self.isEmpty() :
			...
			if ref_card.getNumber() == cards[0].getNumber() + 1 :
...
class SuitPile(abstract.AbstractSimplePile) :
	...
	#vaslidAddCards has to be expended
	...
	def validAddCards(self, cards, contact = True) :
		...
		if self.isEmpty() :
			if cards[0].getNumber() == 13: return True
			return False

		ref_card = self.cards[-1]
		if ref_card.getNumber() - 1 == cards[0].getNumber() :
			return True
		return False
